...
class Parse:
    ...
    def check_applicable_for_job(self, job_description):
        query = f"""Given the job description : {job_description}
        is the candidate with following
        ...
        See if the user passes minimum requirements only. say yes or no in <response></response> tags
        ...
        if response_match and reason_match:
            return response_match.group(1), reason_match.group(1)
        else:
            return None
